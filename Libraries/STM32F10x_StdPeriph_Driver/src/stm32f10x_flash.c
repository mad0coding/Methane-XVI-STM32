...
@code  
 ...
 The table below presents the list of available functions depending on the used STM32F10x devices.  
   ...
   ***************************************************
   ...
   |----------------------------------------------------------------------------------------------------------------------------------|
   |FLASH_WaitForLastOperation_LL          |    Yes     |      Yes      | - Wait for Bank1 last operation (for all devices)              |
...
#else
  /* Wait for last operation to be completed */
  status = FLASH_WaitForLastOperation_LL(EraseTimeout);
...
FLASH_Status FLASH_EraseOptionBytes(void)
{
  ...
  if(status == FLASH_COMPLETE)
  {
    ...
    if(status == FLASH_COMPLETE)
    {
      ...
      /* Wait for last operation to be completed */
      status = FLASH_WaitForLastOperation_LL(ProgramTimeout);
...
/**
  ...
  *         FLASH_ERROR_WRP, FLASH_COMPLETE or FLASH_TIMEOUT.
  */
FLASH_Status FLASH_WaitForLastOperation_LL(uint32_t Timeout)
{ 
  FLASH_Status status = FLASH_COMPLETE;
...
/**
  ...
  * @note   This function can be used for all STM32F10x devices, 
  *         it is equivalent to FLASH_WaitForLastOperation_LL.
  * @param  Timeout: FLASH programming Timeout
  * @retval FLASH Status: The returned value can be: FLASH_ERROR_PG,
  *         FLASH_ERROR_WRP, FLASH_COMPLETE or FLASH_TIMEOUT.
  */
